...
class loops1 implements Runnable{
	...
	public void run(){
		...
		for(i=0; i < 1000000000; i++){	//constant used to avoid a memory hit every comparison. Though one would hope that JVM is smarter than that.
...
*/
			if(x*x + y*y > 1.00000000001){
				miss++;
			}
		}
		hits = (1000000000 - miss)/1000000000.0;	//don't touch the class variable hits inside the loop. That's slower. Calculate the result outside.
	}
}
